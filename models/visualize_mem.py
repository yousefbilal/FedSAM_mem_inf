...
def generate_html_report(
    ...
    for i, idx in enumerate(top_k_indices):
        ...
        img_name = image_paths[idx] # <-- CHANGED: Renamed to img_name for clarity
        label = labels[idx]

        # <-- CHANGED: Construct the full path from the dir and the name
        full_img_path = os.path.join(train_data_dir, img_name)
        # Use absolute path so the browser can find it on your local disk
        abs_img_path = os.path.abspath(full_img_path)
        
        browser_path = abs_img_path
        if browser_path.startswith('/mnt/'):
            # This translates /mnt/c/Users... to C:/Users...
        ...
        # The 'abs_img_path' version worked because it already had a leading /
        src_path = f"file:///{browser_path}"

        html_content += f"""
        <div class="item">
            <img src="{src_path}" alt="Image {idx}" loading="lazy">
...
def main():
    parser = argparse.ArgumentParser(description="Generate a web-based memorization report.")
    parser.add_argument(
        "--results_file",
        type=str,
        required=True,
    ...
    try:
        ...
        print("Please ensure 'utils.model_utils' is importable and your data is in the correct path.")
        sys.exit(1)
    ...
    if len(mem_est) != len(image_paths):
        ...
        print("Make sure you are using the same dataset and the 'sorted()' fix is in your read_data function.")
        sys.exit(1)
