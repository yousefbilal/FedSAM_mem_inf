...
from utils.main_utils import (
    check_init_paths,
    define_server_params,
    create_paths,
    define_client_params,
    resume_run,
    schedule_cycling_lr,
)
...
def main():
    ...
    client_path = "clients/%s.py" % (
        args.client_algorithm + "_client"
...
def training_run(
    ...
    server_params = define_server_params(
        ...
        client_model,
        args.algorithm,
        opt_ckpt=checkpoint["opt_state_dict"]
        if args.load and "opt_state_dict" in checkpoint
        else None,
    )
    ...
    for i in range(start_round, num_rounds):
        ...
        if (
            args.swa and i > swa_start and (i - swa_start) % args.swa_c == 0
        ):  # end of cycle
